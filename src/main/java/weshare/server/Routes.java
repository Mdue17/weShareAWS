...
public class Routes {
    ...
    public static void configure(WeShareServer server) {
        server.routes(() -> {
            ...
            post(PAYMENT_REQUEST,          PayRequestsController.addPaymentRequest);
            get(LOGOUT,                    PersonController.logout);
            get(EXPENSES,                  ExpensesController.view);
            get(NEW_EXPENSES ,             ExpensesController.new_expense);
            get(PAYMENT_REQUESTS_SENT,     PayRequestsController.paymentsent);
            get(PAYMENT_REQUESTS_RECEIVED, PayRequestsController.paymentreceived);
            get(PAYMENT_REQUEST,           PayRequestsController.submitPaymentRequest);
            post(PAYMENT_ACTION, PayRequestsController.paymentAction);
        });
    }
